  ...
  >
    ...
    <div class="px-3 py-1.5 flex-grow flex flex-col">
      <div class="flex items-center justify-between mb-1 min-h-[20px]">
        ...
        <span
          v-else
          class="text-gray-400 font-black text-[9px] uppercase tracking-wider bg-gray-100 px-2 py-0.5 rounded-md"
        >
          {{ $t("lesson_card.locked") }}
        </span>
      </div>
      ...
      <div
        v-if="!isAvailable"
        class="mt-auto pb-1 text-primary/40 font-bold text-xs italic"
      >
        {{ $t("lesson_card.draft") }}
      </div>
      <div v-else class="mt-auto pb-1 text-gray-400 font-bold text-xs italic">
        {{ $t("map.node.locked") }}
      </div>
    </div>
  </div>
