...
// Ensure history data is loaded before rendering
await useAsyncData("lesson-index-history", async () => {
  if (!isLoaded.value) {
      ...
      <div class="overflow-x-auto pb-8 mb-8 no-scrollbar">
        <div class="flex gap-4 min-w-max px-2">
          <a
            ...
            :key="era.id"
            :href="`#${era.id}`"
            class="px-5 sm:px-6 py-2 sm:py-3 bg-white dark:bg-slate-800 rounded-full shadow-md font-black text-sm sm:text-base text-text hover:bg-primary hover:text-white transition-all whitespace-nowrap border-2 border-white dark:border-slate-800 hover:border-primary"
          >
            {{ era.title }}
          </a>
        </div>
