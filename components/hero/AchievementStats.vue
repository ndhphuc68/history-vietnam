...
<template>
  <div class="max-w-7xl mx-auto px-6 mb-16">
    ...
    >
      <!-- Background Glow -->
      <div
        class="absolute -top-24 -right-24 w-64 h-64 bg-primary/10 rounded-full blur-3xl group-hover:bg-primary/20 transition-colors duration-700"
      ></div>
          ...
          <div class="relative inline-flex items-center justify-center mb-4">
            <svg class="w-32 h-32 transform -rotate-90">
              ...
              <circle
                ...
                :stroke-dashoffset="
                  364.4 * (1 - badgeStore.progressPercent / 100)
                "
            ...
            <div class="absolute flex flex-col items-center">
              <span class="text-3xl font-black text-text"
                >{{ badgeStore.progressPercent }}%</span
              >
            </div>
          </div>
          <p class="text-sm font-black text-text/40 uppercase tracking-widest">
            {{ $t("hero_stats.completed") }}
          </p>
        </div>
        ...
        <div class="flex-1 w-full lg:w-auto">
          <div class="flex items-center justify-between mb-8">
            <h3 class="text-2xl font-black text-text">
              {{ $t("hero_stats.progress_title") }}
            </h3>
            <div class="text-lg font-black text-primary">
              {{ badgeStore.earnedCount }} / {{ badgeStore.totalBadges }}
              <span class="text-text/20 ml-1">
                {{ $t("hero_stats.earned") }}
              </span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            >
              <div class="flex items-center gap-3 mb-2">
                ...
                >
                  <Icon
                    :name="rarityIcons[rarity as keyof typeof rarityIcons]"
                  />
                </div>
                <span
                  class="text-xs font-black text-text/40 uppercase tracking-wider"
                  >{{ $t(`common.rarity.${rarity}`) }}</span
                >
              </div>
              <div
                class="text-2xl font-black text-text group-hover/stat:text-primary transition-colors"
              >
                {{ count }}
              </div>
            </div>
