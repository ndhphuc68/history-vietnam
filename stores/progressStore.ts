...
export const useProgressStore = defineStore('progress', () => {
  ...
  const isLessonUnlocked = computed(() => (lessonId: string, eras: any[]) => {
    if (!eras) return false;

    // Find all levels in order
    let allLevels: any[] = [];
    eras.forEach((era: any) => {
      allLevels = [...allLevels, ...era.levels];
    });

    const index = allLevels.findIndex((l: any) => l.lesson === lessonId);
    if (index === -1) return false;

    // First lesson ever is always unlocked
